...
*/
	try
		...
		if ( urlBaseStr == null || urlBaseStr.length() <= 0 )
		{
			...
			// XXX HAA
			int baseEnd = location.indexOf( '/', 8 );
			urlBaseStr = location.substring( 0, baseEnd );
			// String locationHost = HTTP.getHost( location );
			// int locationPort = HTTP.getPort( location );
	...
	public void setDescriptionURI( String uri )
	{
	...
	private final HashMap<String, byte[]> iconBytesMap = new HashMap<String, byte[]>();
	...
	/*
	 * public void output(PrintWriter ps) { ps.println("deviceType = " + getDeviceType()); ps.println("freindlyName = " + getFriendlyName());
	 * ps.println("presentationURL = " + getPresentationURL());
	 * 
	 * DeviceList devList = getDeviceList(); ps.println("devList = " + devList.size());
	 * 
	 * ServiceList serviceList = getServiceList(); ps.println("serviceList = " + serviceList.size());
	 * 
	 * IconList iconList = getIconList(); ps.println("iconList = " + iconList.size()); }
	 * 
	 * public void print() { PrintWriter pr = new PrintWriter(System.out); output(pr); pr.flush(); }
	 */
